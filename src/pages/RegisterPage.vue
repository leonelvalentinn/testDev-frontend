...
const registerUser = (event) => {
  ...
  fetch('http://127.0.0.1:8000/api/create-user', {
    ...
    .catch((error) => {
      ...
      if (error.message === '{"email":["The email has already been taken."]}') {
        notify('Correo ya registrado', 'error')
      }
      if (error.message === '{"name":["The name field is required."]}') {
        notify('El nombre es requerido', 'error')
      }
    })
...
<template>
  <section class="container-full">
    <form class="form" @submit="registerUser($event)">
      ...
      <p class="message">Crea tu cuenta para poder ingresar. </p>
      <label>
